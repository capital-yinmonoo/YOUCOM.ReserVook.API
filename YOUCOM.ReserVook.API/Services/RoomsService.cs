...
//            sql += "   , room_state_class ";
        ...
        public async Task<RequestDataInfo> GetRequestData(RequestDataCondition cond)
        {
            ...
            //{               
            //    sql = "select distinct tfile.reserve_no,tfile.use_date,trn.room_no,mu.machine_no, mc.eregicard_connect_hotelcode,";
            ...
            //    sql += " and tfile.reserve_no=trn.reserve_no inner join mst_user mu on mu.company_no = trn.company_no inner join mst_company mc on mc.company_no = trn.company_no";
            //    sql += " where trn.reserve_no = '"+cond.ReservationNo+"' and trn.room_no = '"+cond.RoomNo+"' and tfile.use_date in (select arrival_date from trn_reserve_basic where reserve_no = '"+cond.ReservationNo+"')";
            ...
            //    sql += " tfile.phone_no,tfile.company_name from trn_name_file tfile inner join trn_reserve_assign trn on trn.company_no = tfile.company_no";
            //    sql += " and tfile.reserve_no=trn.reserve_no inner join mst_user mu on mu.company_no = trn.company_no inner join mst_company mc on mc.company_no = trn.company_no";
            //    sql += " where trn.reserve_no = '"+cond.ReservationNo+"' and trn.room_no = '"+cond.RoomNo+"' and tfile.use_date = '-' and tfile.route_seq = 0";
            //    sql += " and mc.company_no = '"+cond.CompanyNo+"' and trn.use_date in (select arrival_date from trn_reserve_basic where reserve_no = '"+cond.ReservationNo+"')";
            //}
            //#endregion
            ...
            if (CheckExistSetting(cond))
            {
                sql += "select distinct tfile.reserve_no,trn.room_no,mu.machine_no, mc.eregicard_connect_hotelcode,";
                sql += " mc.eregicard_connect_password,tfile.guest_name,tfile.guest_kana,tfile.zip_code,tfile.address,";
                sql += " tfile.phone_no,tfile.company_name from trn_name_file tfile inner join trn_reserve_assign trn on trn.company_no = tfile.company_no";
            ...
            {
                sql = " select distinct tfile.route_seq,trn.route_seq,tfile.reserve_no,trn.room_no,mu.machine_no, ";
                ...
                try
                {
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            info.RoomNo = reader["room_no"].ToString();
                            //info.SystemDate = reader["use_date"].ToString();
